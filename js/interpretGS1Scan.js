...
async function populateTableWithGS1Data(scan, outputNode, table) {
  // Call displayInterpretation to generate the GS1 interpretation and render it in outputNode
  await displayInterpretation(scan, outputNode);

  // Retrieve the generated div with GS1 identifier data
  let identifiersDiv = document.getElementById('identifiers');
  ...
  if (identifiersDiv) {
      ...
      let cell1 = newRow.insertCell(0); // Row number or other data
      let cell2 = newRow.insertCell(1); // SERIAL number
      let cell3 = newRow.insertCell(2); // GTIN
      let cell4 = newRow.insertCell(3); // PROD DATE
      let cell5 = newRow.insertCell(4); // USE BY OR EXPIRY

      // Optional: You can assign some default values to cell1, e.g., row number
      cell1.textContent = table.rows.length;

      // Now extract and assign values to each respective cell
      let serialElement = identifiersDiv.querySelector('p span.aiLabel:contains("SERIAL") + span.aiValue');
      let gtinElement = identifiersDiv.querySelector('p span.aiLabel:contains("GTIN") + span.aiValue');
      let prodDateElement = identifiersDiv.querySelector('p span.aiLabel:contains("PROD DATE") + span.aiValue');
      let useByElement = identifiersDiv.querySelector('p span.aiLabel:contains("USE BY OR EXPIRY") + span.aiValue');

      // Set the cell values based on the extracted elements
      if (serialElement) cell2.textContent = serialElement.textContent;
      if (gtinElement) cell3.textContent = gtinElement.textContent;
      if (prodDateElement) cell4.textContent = prodDateElement.textContent;
      if (useByElement) cell5.textContent = useByElement.textContent;
  } else {
      console.error("Identifiers div not found.");
  }
}
